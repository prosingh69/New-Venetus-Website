...
const Navbar = () => {
  ...
  return (
    <>
      ...
      <nav aria-label="Main Navigation" className={'fixed top-0 left-0 right-0 z-40 bg-[#F8F6F4] transition-all duration-500 ease-in-out ' + (isScrolled ? 'shadow-[0_2px_16px_rgba(0,0,0,0.09)] border-b border-[#EDE0D8]' : 'border-b border-transparent')}>
        <div className={'max-w-screen-xl mx-auto px-5 md:px-10 lg:px-14 flex items-center transition-all duration-500 ease-in-out ' + (isScrolled ? 'h-[60px] md:h-[70px]' : 'h-[100px] md:h-[120px]')}>
          ...
          <a
            ...
            className={'flex md:hidden flex-1 justify-end items-center p-2 rounded-full hover:bg-green-100 transition-all duration-300 ' + (isSidebarOpen ? 'opacity-0 pointer-events-none' : 'opacity-100')}
      ...
      {/* Sidebar Overlay — covers whole screen including navbar */}
      <div className={'fixed inset-0 bg-black/50 z-50 md:hidden backdrop-blur-sm transition-opacity duration-300 ' + (isSidebarOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none')} onClick={toggleSidebar} />

      {/* Mobile Sidebar — sits above overlay */}
      <div className={'fixed top-0 left-0 h-full w-72 bg-[#F8F6F4] shadow-2xl z-[60] transform transition-transform duration-300 ease-in-out md:hidden flex flex-col ' + (isSidebarOpen ? 'translate-x-0' : '-translate-x-full')}>
