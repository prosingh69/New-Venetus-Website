  ...
  return (
    <>
      <nav className='sticky top-0 z-40 flex flex-row-reverse md:flex-row justify-between items-center p-4 bg-[#F9F5F3] border-b border-[#F3E5DE]'>
        ...
        >
          <span className={`absolute w-8 h-0.75 bg-[#B74B21] rounded-full transition-all duration-400 ease-[cubic-bezier(0.84,0.06,0.52,1.8)] ${isSidebarOpen ? 'rotate-40' : '-translate-y-2'
            }`}></span>

          <span className={`absolute w-8 h-0.75 bg-[#B74B21] rounded-full transition-all duration-400 ease-[cubic-bezier(0.84,0.06,0.52,1.8)] ${isSidebarOpen ? 'opacity-0' : 'opacity-100'
            }`}></span>
      ...
      <div
        className={`fixed top-0 left-0 h-full w-70 bg-[#F9F5F3] shadow-2xl z-50 transform transition-transform duration-600 ease-[cubic-bezier(0.62,0.04,0.3,1.56)] md:hidden flex flex-col ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'
          }`}
      >
